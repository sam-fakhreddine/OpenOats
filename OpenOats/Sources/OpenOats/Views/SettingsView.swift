...
private struct TranscriptionSettingsTab: View {
                ...
                Section("Model Storage") {
                    VStack(alignment: .leading, spacing: 6) {
                        HStack {
                            ...
                            Button("Choose...") {
                                selectModelStorageFolder(moveExisting: settings.moveExistingModels)
                            }
                            .font(.system(size: 12))
                            .controlSize(.small)
                        ...
                        Toggle("Move existing models to new location", isOn: $settings.moveExistingModels)
                            .font(.system(size: 12))

                        Text("Choose a different folder to store transcription models. Models can be large files—selecting a different location helps manage storage.")
                            .font(.system(size: 11))
                            .foregroundStyle(.secondary)
                            .fixedSize(horizontal: false, vertical: true)
                    }
    ...
    private func selectModelStorageFolder(moveExisting: Bool = false) {
        let panel = NSOpenPanel()
        panel.canChooseFiles = false
        panel.canChooseDirectories = true
        panel.allowsMultipleSelection = false
        ...
        if panel.runModal() == .OK, let url = panel.url {
            ...
            let isWritable = fm.createFile(atPath: testFile.path, contents: Data(), attributes: nil)
            if isWritable {
                try? fm.removeItem(at: testFile)

                let oldPath = settings.modelStoragePath
                let newPath = url.path

                // Move existing models if requested and path changed
                if moveExisting && !oldPath.isEmpty && oldPath != newPath {
                    moveModels(from: oldPath, to: newPath)
                }

                settings.modelStoragePath = newPath
            } else {
                // Show error alert for non-writable directory
                let alert = NSAlert()
                alert.messageText = "Cannot Write to Selected Folder"
                alert.informativeText = "The selected folder is not writable. Please choose a different location."
    ...
    private func moveModels(from oldPath: String, to newPath: String) {
        let fm = FileManager.default
        let oldURL = URL(fileURLWithPath: oldPath)
        let newURL = URL(fileURLWithPath: newPath)

        // Model subdirectories to migrate
        let modelDirs = ["parakeet", "qwen3-asr", "huggingface"]

        for dir in modelDirs {
            let source = oldURL.appendingPathComponent(dir)
            let destination = newURL.appendingPathComponent(dir)
            ...
            do {
                ...
                // Remove destination if it exists (shouldn't happen for new location)
                ...
                // Log error but don't block the UI - models can be re-downloaded
                print("Failed to move models from \(source.path) to \(destination.path): \(error)")
            }
        }
    }
}
