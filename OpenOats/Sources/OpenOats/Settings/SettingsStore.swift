...
final class SettingsStore {
    ...
    @ObservationIgnored nonisolated(unsafe) private var _moveExistingModels: Bool
    var moveExistingModels: Bool {
        get { access(keyPath: \.moveExistingModels); return _moveExistingModels }
        set {
            withMutation(keyPath: \.moveExistingModels) {
                _moveExistingModels = newValue
                defaults.set(newValue, forKey: "moveExistingModels")
            }
        }
    }

    /// Returns the effective model storage URL. If a custom path is set, uses that; otherwise uses default cache.
    /// Note: Backends append their own subdirectories (e.g., huggingface/hub/mlx-audio) to this base path.
    var modelStorageURL: URL {
        ...
        // Default: ~/Library/Caches (backends append huggingface/hub/mlx-audio)
    ...
    init(storage: SettingsStorage = .live()) {
        ...
        self._moveExistingModels = defaults.bool(forKey: "moveExistingModels")
