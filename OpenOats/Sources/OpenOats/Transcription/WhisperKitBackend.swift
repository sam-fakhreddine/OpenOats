...
///
/// Note: WhisperKit manages its own model cache location. The modelStorageURL parameter is accepted
/// for API consistency but currently uses WhisperKit's default cache location.
final class WhisperKitBackend: TranscriptionBackend, @unchecked Sendable {
    ...
    init(variant: WhisperKitManager.Variant, modelStorageURL: URL? = nil) {
        ...
        // Note: modelStorageURL is accepted for API consistency but WhisperKit uses its own cache
        switch variant {
