...
final class MLXWhisperBackend: TranscriptionBackend, @unchecked Sendable {
    ...
    /// Custom storage directory for MLX models (optional)
    private let modelStorageURL: URL?

    /// Cache directory for MLX models
    private var cacheDirectory: URL {
        modelStorageURL ?? FileManager.default.urls(for: .cachesDirectory, in: .userDomainMask).first!
    ...
    /// - Parameter modelStorageURL: Optional custom directory for model storage. If nil, uses default cache.
    init(modelStorageURL: URL? = nil) {
