...
final class Qwen3Backend: TranscriptionBackend, @unchecked Sendable {
    ...
    init(variant: Qwen3AsrVariant = .f32, modelStorageURL: URL? = nil) {
    ...
    func checkStatus() -> BackendStatus {
        ...
        let exists = Qwen3AsrModels.modelsExist(
            at: OpenOatsLocalModelStore.qwen3Directory(variant: variant, baseDirectory: baseDirectory)
        )
    ...
    func clearModelCache() {
        let baseDirectory = modelStorageURL ?? OpenOatsLocalModelStore.defaultBaseDirectory()
        OpenOatsLocalModelStore.clearQwen3Cache(variant: variant, baseDirectory: baseDirectory)
    }

    func prepare(onStatus: @Sendable (String) -> Void, onProgress: @escaping @Sendable (Double) -> Void) async throws {
        let baseDirectory = modelStorageURL ?? OpenOatsLocalModelStore.defaultBaseDirectory()
        let modelsDirectory = OpenOatsLocalModelStore.qwen3Directory(variant: variant, baseDirectory: baseDirectory)
