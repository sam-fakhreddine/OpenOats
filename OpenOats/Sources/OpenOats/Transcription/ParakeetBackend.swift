...
final class ParakeetBackend: TranscriptionBackend, @unchecked Sendable {
    ...
    init(version: AsrModelVersion, customVocabulary: String = "", modelStorageURL: URL? = nil) {
    ...
    func checkStatus() -> BackendStatus {
        let baseDirectory = modelStorageURL ?? OpenOatsLocalModelStore.defaultBaseDirectory()
        let exists = AsrModels.modelsExist(
            at: OpenOatsLocalModelStore.parakeetDirectory(for: version, baseDirectory: baseDirectory),
            version: version
        )
    ...
    func clearModelCache() {
        let baseDirectory = modelStorageURL ?? OpenOatsLocalModelStore.defaultBaseDirectory()
        OpenOatsLocalModelStore.clearParakeetCache(for: version, baseDirectory: baseDirectory)
    }

    func prepare(onStatus: @Sendable (String) -> Void, onProgress: @escaping @Sendable (Double) -> Void) async throws {
        let models: AsrModels
        let baseDirectory = modelStorageURL ?? OpenOatsLocalModelStore.defaultBaseDirectory()
        let modelsDirectory = OpenOatsLocalModelStore.parakeetDirectory(for: version, baseDirectory: baseDirectory)
